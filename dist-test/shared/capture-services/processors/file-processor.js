...
class FileProcessor {
     ...
     * 处理文本文件
     */
    async processTextFile(item, result) {
     ...
     * 处理文档文件
     */
    async processDocumentFile(item, result) {
     ...
     * 处理图片文件
     */
    async processImageFile(item, result) {
        const { mimeType, fileName } = item.content.file;
     ...
     * 处理通用文件
     */
    async processGenericFile(item, result) {
        const { mimeType, fileName } = item.content.file;
        // 对于不支持的文件类型，只提供基本信息
     ...
     * 分析Markdown结构
     */
    async analyzeMarkdownStructure(markdown) {
        try {
            // 使用AI服务分析Markdown结构
     ...
     * 从文件信息生成标签
     */
    generateTagsFromFileInfo(fileName, mimeType) {
        const tags = [];
        // 从MIME类型生成标签
     ...
     * 从MIME类型获取文档类型
     */
    getDocumentTypeFromMimeType(mimeType) {
        switch (mimeType) {
...
// 创建全局文件处理器实例
exports.globalFileProcessor = new FileProcessor();
