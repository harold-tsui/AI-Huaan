...
export class ConfigManager {
  ...
  private mergeWithEnvVars(defaultConfig: AppConfig): AppConfig {
    ...
    if (process.env.APP_VERSION) config.app.version = process.env.APP_VERSION;
    if (process.env.NODE_ENV) config.app.environment = process.env.NODE_ENV as 'development' | 'test' | 'production';
    if (process.env.PORT) config.app.port = parseInt(process.env.PORT);
    if (process.env.HOST) config.app.host = process.env.HOST;
    if (process.env.BASE_URL) config.app.baseUrl = process.env.BASE_URL;
    
    // 数据库配置 - PostgreSQL
    if (process.env.POSTGRES_HOST) config.database.postgres.host = process.env.POSTGRES_HOST;
    if (process.env.POSTGRES_PORT) config.database.postgres.port = parseInt(process.env.POSTGRES_PORT);
    if (process.env.POSTGRES_USER) config.database.postgres.username = process.env.POSTGRES_USER;
    if (process.env.POSTGRES_PASSWORD) config.database.postgres.password = process.env.POSTGRES_PASSWORD;
    if (process.env.POSTGRES_DB) config.database.postgres.database = process.env.POSTGRES_DB;
    if (process.env.POSTGRES_SSL) config.database.postgres.ssl = process.env.POSTGRES_SSL === 'true';
    if (process.env.POSTGRES_POOL_SIZE) config.database.postgres.poolSize = parseInt(process.env.POSTGRES_POOL_SIZE);
    
    // 数据库配置 - Redis
    if (process.env.REDIS_HOST) config.database.redis.host = process.env.REDIS_HOST;
    if (process.env.REDIS_PORT) config.database.redis.port = parseInt(process.env.REDIS_PORT);
    if (process.env.REDIS_PASSWORD) config.database.redis.password = process.env.REDIS_PASSWORD;
    if (process.env.REDIS_DB) config.database.redis.db = parseInt(process.env.REDIS_DB);
    if (process.env.REDIS_PREFIX) config.database.redis.prefix = process.env.REDIS_PREFIX;
    ...
    if (process.env.NEO4J_URI || process.env.NEO4J_USER || process.env.NEO4J_PASSWORD) {
      config.database.neo4j = {
        ...
        vectorSearch: {
          ...
          similarity: (process.env.NEO4J_VECTOR_SIMILARITY || 'cosine') as 'cosine' | 'euclidean',
        },
        plugins: {
        ...
        logLevel: (process.env.NEO4J_LOG_LEVEL || 'info') as 'error' | 'warn' | 'info' | 'debug' | 'trace',
      };
    }
    ...
    if (process.env.AI_DEFAULT_PROVIDER) {
      config.ai.routing.defaultProvider = process.env.AI_DEFAULT_PROVIDER as 'openai' | 'anthropic' | 'local';
    }
    if (process.env.AI_FALLBACK_PROVIDER) {
      config.ai.routing.fallbackProvider = process.env.AI_FALLBACK_PROVIDER as 'openai' | 'anthropic' | 'local';
    }
    
    // 存储配置
    if (process.env.STORAGE_TYPE) config.storage.type = process.env.STORAGE_TYPE as 'local' | 's3' | 'minio';
    if (process.env.STORAGE_PATH) config.storage.basePath = process.env.STORAGE_PATH;
    ...
    if (process.env.LOG_CONSOLE) config.logging.console = process.env.LOG_CONSOLE === 'true';
    if (process.env.LOG_FILE) config.logging.file = process.env.LOG_FILE === 'true';
    if (process.env.LOG_FILE_PATH) config.logging.filePath = process.env.LOG_FILE_PATH;
    if (process.env.LOG_MAX_SIZE) config.logging.maxSize = process.env.LOG_MAX_SIZE;
    if (process.env.LOG_MAX_FILES) config.logging.maxFiles = parseInt(process.env.LOG_MAX_FILES);
    if (process.env.LOG_FORMAT) config.logging.format = process.env.LOG_FORMAT as 'json' | 'simple' | 'detailed';
    
    // 安全配置 - JWT
    if (process.env.JWT_SECRET) config.security.jwt.secret = process.env.JWT_SECRET;
    if (process.env.JWT_EXPIRES_IN) config.security.jwt.expiresIn = process.env.JWT_EXPIRES_IN;
    if (process.env.JWT_REFRESH_EXPIRES_IN) config.security.jwt.refreshExpiresIn = process.env.JWT_REFRESH_EXPIRES_IN;
    
    // 安全配置 - 加密
    if (process.env.ENCRYPTION_ALGORITHM) config.security.encryption.algorithm = process.env.ENCRYPTION_ALGORITHM;
    if (process.env.ENCRYPTION_KEY) config.security.encryption.key = process.env.ENCRYPTION_KEY;
    
    // 安全配置 - CORS
    if (process.env.CORS_ORIGINS) config.security.cors.origins = process.env.CORS_ORIGINS.split(',');
    if (process.env.CORS_METHODS) config.security.cors.methods = process.env.CORS_METHODS.split(',');
   ...
   * @returns 是否成功
   */
  public saveConfig(filePath?: string, options: { pretty?: boolean; backup?: boolean } = {}): boolean {
    this.ensureInitialized();
...
export function validateNeo4jConfig(config: Neo4jConfig): void {
  const logger = new Logger('Neo4jConfig');
  
  // 验证必要字段
  const requiredFields = ['uri', 'username', 'password'] as const;
