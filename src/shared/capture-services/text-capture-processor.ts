...
import { IAIService, MessageRole, ContentType, MessageContent, Message } from '../ai-services/types';
 ...
 * 文本捕获处理器
 */
export class TextCaptureProcessor extends BaseCaptureProcessor {
  ...
  constructor(aiService?: IAIService, knowledgeGraphService?: IKnowledgeGraphService) {
    super(
      ...
      'Processes plain text content, analyzes it, and creates knowledge graph nodes',
  ...
  protected async processTextContent(item: CaptureItem): Promise<CaptureItemProcessingResult> {
    ...
    try {
      ...
      // 更新捕获项内容
      let updatedContent: CaptureItemContent = {
        ...item.content,
        text,
      };
      
      // 如果有AI服务，生成摘要、标签和Markdown格式
      let processingResult: CaptureItemProcessingResult = {
        success: true,
   ...
   * @param title 标题
   */
  private async enrichContentWithAI(text: string, title: string): Promise<CaptureItemProcessingResult> {
      ...
      const summaryResult = await this.aiService.chat(
        [
          { 
            ...
            content: [{ type: ContentType.TEXT, text: 'You are a helpful assistant that creates concise, informative summaries.' }]
          },
          { 
            role: MessageRole.USER, 
            content: [{ type: ContentType.TEXT, text: summaryPrompt }]
          }
        ]
      );
...
Text: ${truncatedText}`;
      const tagsResult = await this.aiService.chat(
        [
          { 
            role: MessageRole.SYSTEM, 
            content: [{ type: ContentType.TEXT, text: 'You are a helpful assistant that extracts relevant tags and keywords.' }]
          },
          { 
            role: MessageRole.USER, 
            content: [{ type: ContentType.TEXT, text: tagsPrompt }]
          }
        ]
      );
...
Text: ${truncatedText}`;
      const markdownResult = await this.aiService.chat(
        [
          { 
            role: MessageRole.SYSTEM, 
            content: [{ type: ContentType.TEXT, text: 'You are a helpful assistant that formats text into well-structured Markdown.' }]
          },
          { 
            role: MessageRole.USER, 
            content: [{ type: ContentType.TEXT, text: markdownPrompt }]
          }
        ]
      );
      
      // 从消息内容中提取文本
      const getTextFromContent = (content: MessageContent | MessageContent[]): string => {
   ...
   * @param text 文本内容
   */
  private async extractAndLinkEntities(sourceNodeId: string, text: string): Promise<void> {
    ...
    try {
      ...
      const entitiesResult = await this.aiService.chat([
        { role: MessageRole.SYSTEM, content: { type: ContentType.TEXT, text: 'You are a helpful assistant that extracts named entities from text.' } },
        { role: MessageRole.USER, content: { type: ContentType.TEXT, text: entitiesPrompt } }
      ]);
