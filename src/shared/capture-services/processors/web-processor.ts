 ...
 * 网页处理器实现
 */
export class WebProcessor implements ICaptureProcessor {
   ...
   */
  constructor() {
    this.logger = new Logger('WebProcessor');
  }
   ...
   * 从HTML提取链接
   */
  private extractLinksFromHtml(html: string): { url: string; text: string; isExternal: boolean }[] {
   ...
   * 从HTML提取图片
   */
  private extractImagesFromHtml(html: string): { url: string; alt: string; width?: number; height?: number }[] {
    ...
    try {
      ...
      for (let i = 0; i < imgElements.length; i++) {
        ...
        if (url) {
          const imageInfo: { url: string; alt: string; width?: number; height?: number } = {
            url,
   ...
   * 分析网页结构
   */
  private async analyzeWebPageStructure(html: string): Promise<{ structure: string; headings: string[]; mainContent: string; navigation: string[]; }> {
    try {
      // 使用AI服务分析网页结构
      const result = await globalAIRoutingService.chat(
        [
          { role: MessageRole.SYSTEM, content: { type: ContentType.TEXT, text: '你是一个专业的网页分析助手，擅长分析网页结构和内容组织。请分析提供的HTML代码，识别主要结构元素（如标题、导航、主要内容区、侧边栏、页脚等）及其层次关系。以JSON格式返回分析结果。' } },
          { role: MessageRole.USER, content: { type: ContentType.TEXT, text: `请分析以下HTML代码的结构：\n\n${html.length > 8000 ? html.substring(0, 8000) + '...' : html}` } },
        ],
        { temperature: 0.2 }
      );
...
// 创建全局网页处理器实例
export const globalWebProcessor = new WebProcessor();