...
exports.globalTextProcessor = exports.TextProcessor = void 0;
const logger_1 = require("../../../utils/logger");
const ai_services_1 = require("../../ai-services");
const prompt_manager_1 = require("../../ai-services/prompt-manager");
...
class TextProcessor {
     ...
     */
    constructor() {
        this.logger = new logger_1.Logger('TextProcessor');
    }
    /**
     * 处理捕获项
     */
    ...
    async generateSummary(text) {
        try {
            ...
            const result = await ai_services_1.globalAIRoutingService.chat([
                ...
                { role: types_1.MessageRole.USER, content: { type: types_1.ContentType.TEXT, text: prompt } },
            ], { temperature: 0.3 });
    ...
    async extractKeyInsights(text) {
        try {
            // 使用AI服务提取关键见解
            const result = await ai_services_1.globalAIRoutingService.chat([
                { role: types_1.MessageRole.SYSTEM, content: { type: types_1.ContentType.TEXT, text: '你是一个专业的内容分析助手，擅长从文本中提取关键见解和重要观点。请提取5-7个关键见解，每个见解应简洁明了，不超过100字。以JSON数组格式返回结果。' } },
                { role: types_1.MessageRole.USER, content: { type: types_1.ContentType.TEXT, text: `请从以下文本中提取关键见解：\n\n${text.length > 8000 ? text.substring(0, 8000) + '...' : text}` } },
            ], { temperature: 0.3 });
            // 尝试解析JSON响应
            try {
                const messageContent = result.message.content;
    ...
    async extractEntities(text) {
        try {
            // 使用AI服务提取实体
            const result = await ai_services_1.globalAIRoutingService.chat([
                { role: types_1.MessageRole.SYSTEM, content: { type: types_1.ContentType.TEXT, text: '你是一个专业的实体识别助手，擅长从文本中识别人物、组织、地点、概念等实体。请以JSON格式返回结果，包含实体名称、类型和相关性分数（0-1）。' } },
                { role: types_1.MessageRole.USER, content: { type: types_1.ContentType.TEXT, text: `请从以下文本中识别重要实体：\n\n${text.length > 8000 ? text.substring(0, 8000) + '...' : text}` } },
            ], { temperature: 0.2 });
            // 尝试解析JSON响应
            try {
                const messageContent = result.message.content;
    ...
    async analyzeSentiment(text) {
        try {
            // 使用AI服务分析情感
            const result = await ai_services_1.globalAIRoutingService.chat([
                { role: types_1.MessageRole.SYSTEM, content: { type: types_1.ContentType.TEXT, text: '你是一个专业的情感分析助手，擅长分析文本的情感倾向。请分析文本的情感，并返回一个-1到1之间的分数（-1表示极度负面，0表示中性，1表示极度正面）和一个情感标签（如"积极"、"中性"、"消极"）。以JSON格式返回结果。' } },
                { role: types_1.MessageRole.USER, content: { type: types_1.ContentType.TEXT, text: `请分析以下文本的情感：\n\n${text.length > 8000 ? text.substring(0, 8000) + '...' : text}` } },
            ], { temperature: 0.1 });
            // 尝试解析JSON响应
            try {
                const messageContent = result.message.content;
    ...
    async extractTopics(text) {
        try {
            // 使用AI服务提取主题
            const result = await ai_services_1.globalAIRoutingService.chat([
                { role: types_1.MessageRole.SYSTEM, content: { type: types_1.ContentType.TEXT, text: '你是一个专业的主题分析助手，擅长从文本中提取主要主题和话题。请提取3-5个主题，并为每个主题提供一个相关性分数（0-1）。以JSON格式返回结果。' } },
                { role: types_1.MessageRole.USER, content: { type: types_1.ContentType.TEXT, text: `请从以下文本中提取主要主题：\n\n${text.length > 8000 ? text.substring(0, 8000) + '...' : text}` } },
            ], { temperature: 0.2 });
            // 尝试解析JSON响应
            try {
                const messageContent = result.message.content;
    ...
    async suggestTags(text) {
        try {
            // 使用AI服务建议标签
            const result = await ai_services_1.globalAIRoutingService.chat([
                { role: types_1.MessageRole.SYSTEM, content: { type: types_1.ContentType.TEXT, text: '你是一个专业的标签生成助手，擅长为文本内容生成相关标签。请生成5-10个相关标签，每个标签应简短（1-3个词）。以JSON数组格式返回结果。' } },
                { role: types_1.MessageRole.USER, content: { type: types_1.ContentType.TEXT, text: `请为以下文本生成标签：\n\n${text.length > 8000 ? text.substring(0, 8000) + '...' : text}` } },
            ], { temperature: 0.3 });
            // 尝试解析JSON响应
            try {
                const messageContent = result.message.content;
                ...
                if (contentText.includes(',')) {
                    return contentText.split(',').map((tag) => tag.trim()).filter(Boolean);
                }
                else {
    ...
    getSupportedSourceTypes() {
        return [types_2.CaptureSourceType.TEXT];
    }
    /**
     * 获取处理器名称
     */
...
// 创建全局文本处理器实例
exports.globalTextProcessor = new TextProcessor();
