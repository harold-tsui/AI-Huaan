 ...
 * 基于Neo4j图数据库的知识图谱服务实现，适用于生产环境
 */
class Neo4jKnowledgeGraphService extends base_knowledge_graph_service_1.BaseKnowledgeGraphService {
     ...
     * 转换Neo4j节点为应用节点
     */
    convertNeo4jNodeToNode(neo4jNode) {
        const neo4jNodeProps = neo4jNode.properties;
     ...
     * 转换Neo4j关系为应用关系
     */
    convertNeo4jRelationshipToRelationship(record) {
        const neo4jRel = record.get('r');
        const sourceNode = record.get('source');
        const targetNode = record.get('target');
        const neo4jRelProps = neo4jRel.properties;
        const sourceNodeProps = sourceNode.properties;
     ...
     * 导出为JSON格式
     */
    async exportToJson(filePath, nodes, relationships) {
        const fs = require('fs');
     ...
     * 导出为CSV格式
     */
    async exportToCsv(dirPath, nodes, relationships) {
        const fs = require('fs');
        const path = require('path');
        const { promisify } = require('util');
        const writeFile = promisify(fs.writeFile);
     ...
     * 导出为GraphML格式
     */
    async exportToGraphML(filePath, nodes, relationships) {
        const fs = require('fs');
        const path = require('path');
        const { promisify } = require('util');
        const writeFile = promisify(fs.writeFile);
